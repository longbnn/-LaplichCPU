...
{
    public partial class CPUProcess : Form
    {
        Graphics graphics;
        int x; int y;
        ...
        public void SJF()
        {
            ...
            for (int i = 1; i < 5; i++)
            {
                for (int j = i; j < 4; j++)
                {
                    if (listprocess[j + 1].txuathien <= listprocess[i].tnapcpu && listprocess[j + 1].tthuchien < listprocess[i].tthuchien)
                    {
                        Swap(listprocess, j, j + 1);
                    }
                    listprocess[j].tnapcpu = listprocess[j - 1].tendcpu;
                    if (listprocess[j].tnapcpu < listprocess[j].txuathien)
                    {
                        listprocess[j].tnapcpu = listprocess[j].txuathien;
                    }
                    listprocess[j].tendcpu = listprocess[j].tnapcpu + listprocess[j].tthuchien;
        ...
        {           
            ...
            for (int i = 0; i < listprocess.Count - 1; i++)
            {
                ...
                for(int j = i+1; j < listprocess.Count - 1; j++)
                {
        ...
        private void CPUProcess_Load(object sender, EventArgs e)
        {
